...
const Description = ({ pageId, sectionIndex }) => {
      ...
      try {
        ...
        if (pageContent) {
          const updatedContent = {
            ...
            paragraphs: pageContent.paragraphs.map((p) => p.replace(/{city}/g, city)),
            listItem1: pageContent.listItem1 || [],
            secondTitle: pageContent.secondTitle?.replace(/{city}/g, city) || '',
            secondParagraphs: pageContent.secondParagraphs?.map((p) => p.replace(/{city}/g, city)) || [],
  ...
  const applyHighlights = (text, highlights) => {
    ...
    const validHighlights = highlights.filter((word) => typeof word === 'string' && word.trim() !== '')
    if (validHighlights.length === 0) return text
    
    const regex = new RegExp(`(${validHighlights.join('|')})`, 'gi')
    return text.split(regex).map((part, index) => {
      if (validHighlights.some((word) => word.toLowerCase() === part.toLowerCase())) {
        return <span key={index} className={styles.highlightDescJson}>{part}</span>
      }
      return part
    })
  }
  ...
  return (
    <div className={`${styles.descriptionContainer} ${sectionIndex % 2 === 0 ? styles.videoLeft : styles.videoRight}`}>
      {content.videoUrl && (
