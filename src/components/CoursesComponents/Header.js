...
const DSHeader = ({ pageId, pageType }) => {
  ...
  const [formData, setFormData] = useState({ countryCode: '+91' });
    ...
    const fetchData = async () => {
      try {
        ...
        if (pageData) {
          ...
          pageData.alumni = pageData.alumni.map((company) => ({ ...company, name: company.name.replace(/{city}/g, city) }));
          pageData.buttons = pageData.buttons.map((button) => ({ ...button, text: button.text.replace(/{city}/g, city) }));
  ...
  const handleChange = (event) => {
    ...
    if (name === 'phone' && value.startsWith('+91') && value.length > 13) return;
  ...
  const handleSubmit = async (event) => {
    ...
    if (!emailRegex.test(formData.email)) {
      ...
      const response = await fetch('https://serverbackend-0nvg.onrender.com/api/submit', {
        method: 'POST',
    ...
    return useMemo(() => {
      ...
      return data.form.inputs.map((input, index) => {
        return input.countryCode ? (
          <div key={index} className={style.phoneInputItDs }>
                <select
                    id="countryCode"
                    name="countryCode"
  ...
  return (
    <div className={styles.containerItDsHeader}>
      ...
      <video className={styles.backgroundVideoHeader} src={data.backgroundVideo}  no repeat autoPlay muted loop loading="lazy" />

      <div className={styles.leftSectionItDs}>
        <h1><span className={styles.dsHeaderSpan}>{data.title}</span></h1>
        <h2><span className={styles.dsHeaderSpan2}>{data.subtitle}</span></h2>
        <p>{data.description}</p>
        <ul className={styles.featuresItDs}>{data.features.map((feature, index) => 
          <li className={styles.featuresItDsli} key={index}>{feature}</li>)}</ul>
        <div className={styles.alumniItDs}><span>Find our Alumni at -</span>
        <div className={styles.alumniLogosItDs}>{data.alumni.map((company, index) => 
          <img key={index} src={company.logo} alt={`${company.name} logo`} />)}</div></div>
        <div className={styles.buttons}>{data.buttons.map((button, index) => 
          <button key={index} className={index === 0 ? styles.buttonStyle1 : styles.buttonStyle2} onClick={handleButtonClick}>{button.text}</button>)}</div>
      </div>
      ...
      <div className={styles.rightSectionItDs}>
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <button type="submit" 
          className={styles.submitButtonItDs} disabled={isSubmitting}>{data.form.submitText}</button>
